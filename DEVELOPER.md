...
- Datasource import V1 deliberately round-trips only normalized inventory fields (`uid`, `name`, `type`, `access`, `url`, `isDefault`) and does not attempt to recreate secret-bearing datasource settings.
...
`inspect-export` and `inspect-live` now also accept `--output-format` as a single explicit output selector. `text`, `table`, and `json` map to the legacy summary modes, while `report-table`, `report-csv`, `report-json`, `report-tree`, `report-tree-table`, `governance`, and `governance-json` map to the corresponding report/governance modes. This flag is intentionally mutually exclusive with legacy `--json`, `--table`, and `--report` so operators do not have to reason about mixed output selectors.
