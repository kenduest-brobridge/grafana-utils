...
def emit_header(lines: list[str], stem: str, title: str) -> None:
    lines.extend(
        [
...
def load_subcommands(spec: NamespaceSpec) -> list[CommandDocPage]:
    if spec.sub_docs:
...
def generate_namespace_manpage(spec: NamespaceSpec) -> str:
    ...
    subcommands = load_subcommands(spec)

    lines: list[str] = []
    emit_header(lines, spec.stem, spec.title)
...
def generate_top_level_manpage() -> str:
...
def generate_manpages() -> dict[str, str]:
    """Return docs/man-relative output paths and generated roff contents."""
    outputs = {"grafana-util.1": generate_top_level_manpage()}
    for spec in NAMESPACE_SPECS:
        outputs[f"{spec.stem}.1"] = generate_namespace_manpage(spec)
    return outputs
