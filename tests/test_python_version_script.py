...
class SetVersionScriptTests(unittest.TestCase):
    ...
    def test_version_script_release_version_updates_all_version_files(self):
        ...
        self.assertIn('version = "0.2.18"', (root / "pyproject.toml").read_text(encoding="utf-8"))
        self.assertIn('version = "0.2.18"', (root / "rust" / "Cargo.toml").read_text(encoding="utf-8"))
    ...
    def test_version_script_sync_from_file_accepts_python_dev_notation(self):
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn('version = "0.2.19.dev3"', (root / "pyproject.toml").read_text(encoding="utf-8"))
    ...
    def test_version_script_dry_run_leaves_files_unchanged(self):
        ...
        self.assertEqual((root / "VERSION").read_text(encoding="utf-8").strip(), "0.2.17")
