...
def run_export_dashboards(args, deps):
    ...
    for (
        ...
        if export_raw:
            permission_documents.extend(
                deps["collect_permission_export_documents"](
                    scoped_client,
                    summaries,
                    scoped_folder_inventory,
                )
    ...
    if export_raw:
        ...
        raw_permissions_path = (
            raw_variant_dir / deps["DASHBOARD_PERMISSION_BUNDLE_FILENAME"]
        )
        ...
        raw_permissions = {
            ...
            "schemaVersion": 1,
            "summary": {
                "resourceCount": len(permission_documents),
                "dashboardCount": len(
                    [
                        item
                        for item in permission_documents
            ...
            "resources": permission_documents,
        }
        ...
        if not args.dry_run:
            ...
            deps["write_json_document"](raw_permissions, raw_permissions_path)
    ...
    if raw_permissions_path is not None:
        summary_parts.append(f"Raw permissions: {raw_permissions_path}")
