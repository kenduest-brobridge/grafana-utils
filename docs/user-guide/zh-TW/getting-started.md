...
VERSION=0.9.1 BIN_DIR="$HOME/.local/bin" \
  curl -sSL https://raw.githubusercontent.com/kenduest-brobridge/grafana-util/main/scripts/install.sh | sh
```
...
| 模式 | 適合情境 | 範例 |
| :--- | :--- | :--- |
| 直接基本驗證 (Basic Auth) | 本機引導、緊急接手 (break-glass)、管理員作業 | `grafana-util observe live --url http://localhost:3000 --basic-user admin --prompt-password --output-format yaml` |
| `config profile` | 連線已確認後的日常維運、CI、可重複執行工作流 | `grafana-util observe live --profile prod --output-format yaml` |
| 直接 Token 驗證 | 單一組織或權限受限的 API 自動化 | `grafana-util observe overview --url http://localhost:3000 --token "$GRAFANA_API_TOKEN" --output-format yaml` |
...
```
`observe live` 會直接查詢 Grafana，而且現在統一用 `--output-format` 來指定輸出格式。

### 2. `observe overview` 入口
```bash
# 用途：2. observe overview 入口。
...
```
`observe overview live` 是共用 live status 路徑的人類導向包裝。要看可讀摘要可用 `--output-format yaml`，想進互動式工作台就用 `--output-format interactive`。
...
`grafana-util advanced dashboard live browse` 會在終端機中顯示 live dashboard tree；`observe overview live --output-format interactive` 則會顯示互動式的整體總覽。
