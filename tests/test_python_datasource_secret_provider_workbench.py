...
class DatasourceSecretProviderWorkbenchTests(unittest.TestCase):
    def test_datasource_secret_provider_module_parses_as_python39_syntax(self):
        source = MODULE_PATH.read_text(encoding="utf-8")
        ast.parse(source, filename=str(MODULE_PATH), feature_version=(3, 9))

    def test_datasource_secret_provider_collect_provider_references_rejects_opaque_secret_replay(self):
    ...
    def test_datasource_secret_provider_build_provider_plan_shapes_review_summary(self):
    ...
    def test_datasource_secret_provider_iter_provider_names_deduplicates_names(self):
        plan = provider_workbench.build_provider_plan(
            {
